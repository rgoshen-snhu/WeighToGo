...
**Scope:** New `cliff.toml`, new `CHANGELOG.md`; `README.md` cross-links

**Summary:**
Introduced [git-cliff](https://git-cliff.org/) as the project's CHANGELOG generator. Two artifacts:

- **`cliff.toml`** — configured for Keep-a-Changelog format with conventional-commit type → section mapping (`feat` → Added, `fix` → Fixed, `perf` → Performance, `refactor` → Changed, `docs` → Documentation, commits whose body matches `security` → Security). Routine commit types — `chore`, `ci`, `build`, `style`, `test`, `revert`, `Merge` — are filtered from user-facing notes. `tag_pattern = "v[0-9]+\\.[0-9]+\\.[0-9]+$"` constrains release boundaries to semver-only tags, and `skip_tags = "android"` ensures the `v1.0.0-android` mobile artifact does not appear as a web-rebuild release. The remote block points at the `rgoshen-snhu/WeighToGo` repo so PR links resolve correctly.
- **`CHANGELOG.md`** — generated by `git-cliff --tag v0.1.0 v1.0.0-android..HEAD`. Lists every M2-era feature, fix, doc, and security commit under v0.1.0, with `**scope:** subject` prefixes preserved. The header explicitly notes that the Android artifact is preserved separately under the `v1.0.0-android` tag and is intentionally out of scope for this changelog.

Updated `README.md` to add `CHANGELOG.md` to the Documentation table and Repository Layout tree.

**Rationale:**
The user explicitly requested CHANGELOG creation and "automate as much as possible" for releases. SUMMARY.md is the engineering narrative log (rationale, bug-fix context, internal audience); CHANGELOG.md is the user-facing release notes per version (Added/Changed/Fixed/Security, external audience). Keeping them separate matches industry convention and serves different readers.

Considered alternatives:
- **`release-please`** — rejected. Its PR-driven release flow fights the milestone-tied tagging cadence (v0.1.0 → v0.2.0 → v1.0.0 are planned per SRS §5.6, not commit-derived).
- **`conventional-changelog-cli`** — rejected. Requires Node at the repo root just for changelog generation; git-cliff is a single self-contained binary.
- **Hand-curating from SUMMARY.md** — rejected. Duplicates the SUMMARY narrative work and provides no automation path for future releases.

**References:**
- Tool: <https://git-cliff.org/>
- Issue: GH-15
